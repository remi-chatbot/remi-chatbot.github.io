...
// Function to save full conversation transcript to Firebase
const saveConversationTranscript = async (userId: string, transcriptContent: string) => {
  try {
    // Get the count of existing transcripts for this user
    const q = query(
      collection(db, 'conversation_transcripts'),
      where('userId', '==', userId)
    );
    const querySnapshot = await getDocs(q);
    const sessionNumber = querySnapshot.size + 1;

    // Add new transcript with session number and timestamp
    await addDoc(collection(db, 'conversation_transcripts'), {
      userId,
...
// Then modify handleEndConversation with this
const handleEndConversation = async () => {
  try {
    // 1. Disconnect active conversation
    await disconnectConversation();

    // 2. Close confirmation modal & open summary modal
    setShowEndConfirmation(false);
    setSummaryModalOpen(true);
    setSummaryContent('Generating summary...');

    // 3. Collect and format the entire conversation as a transcript
    const transcriptContent = items
      .filter(item => item.formatted.text || item.formatted.transcript || item.formatted.output)
      .map(item => {
        let role = item.role || 'system';
        let content = '';
    ...
    // 5. If user is logged in, save both summary and transcript
    if (user) {
      try {
        if (summaryContent) {
          await saveNewSummary(user.uid, summaryContent); // Save summary and show in UI
        }
        if (transcriptContent) {
          await saveConversationTranscript(user.uid, transcriptContent); // Just save transcript silently
        }
      } catch (error) {
